...
bool Lexical::lexical_class::is_varaible() {
     if (position >= source.length()) {
        return false;
    }
...
std::vector<Token::token_class> Lexical::lexical_class::tokenize() {
    ...
    while (position < source.length()) {
        ...
        tokens.push_back({Token::token_type::ERROR, std::string(1, current), start_line, start_column});
        next_token();
    }
...
std::string Lexical::lexical_class::token_type_name(Token::token_class token) {
    switch (token.type) {
        ...
        case Token::token_type::ERROR: return "ERROR";
